...
const mockFromSeed = (seed: number) => {
  const rand = (offset: number, span: number) => (seed * (offset + 3) * 9301 + 49297) % 233280 % span;
  const heartRate = 70 + (rand(1, 20) % 20);
  const systolic = 110 + (rand(2, 16) % 16);
  const diastolic = 70 + (rand(3, 12) % 12);
  const glucose = 90 + (rand(4, 50) % 50);
  const temperature = 36.5 + (rand(5, 7) % 7) / 10;
  const respiratory = 12 + (rand(6, 7) % 7);
  const oxygen = 96 + (rand(7, 4) % 4);
  return {
    vitalsData: {
      ...
      spo2: oxygen,
      datetime: new Date(Date.now() - (rand(8, 40) % 40) * 60000).toISOString(),
    },
    glucoseData: { value_mgdl: glucose },
  };
};

export function LatestVitals({ patientId, showLastReading = false, useMockData = false }: LatestVitalsProps) {
  ...
  useEffect(() => {
    if (useMockData) {
      ...
      const updateMock = () => {
        const mock = mockFromSeed(seed + Date.now());
        setVitalsData(mock.vitalsData);
        setGlucoseData(mock.glucoseData);
      };
      updateMock();
