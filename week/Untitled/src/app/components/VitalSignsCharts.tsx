...
export function VitalSignsCharts({
  ...
  const resolvedPatientId = useMemo(() => {
    ...
    return params.get('patient_id') || '00001';
  }, [patientId]);
    ...
    const fetchSeries = async () => {
      try {
        const res = await fetch(
          `/api/vitals_series?patient_id=${encodeURIComponent(resolvedPatientId)}&period=${timeRange}`,
          { credentials: 'same-origin' }
        );
        if (!res.ok) throw new Error('vitals series request failed');
  ...
  }, [resolvedPatientId, timeRange]);

  const fallbackData = useMemo(() => buildMockSeries(7, timeRange), [timeRange]);
