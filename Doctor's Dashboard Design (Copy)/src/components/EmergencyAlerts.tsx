...
export function EmergencyAlerts({ onViewPatient }: EmergencyAlertsProps) {
  ...
  const handleDeleteAlert = (alertId: number) => {
    setEmergencyAlerts(emergencyAlerts.filter(alert => alert.id !== alertId));
  };
  ...
  const AlertCard = ({ alert }: { alert: typeof emergencyAlerts[0] }) => (
    <div className={`rounded-lg py-1.5 px-3 shadow-sm border-l-4 ${
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="font-medium text-sm whitespace-nowrap w-[180px]">{alert.reason}</span>
        ...
        <span className="text-sm text-gray-700 whitespace-nowrap w-[160px] text-right">{alert.detectedTime} ({alert.timeAgo})</span>
